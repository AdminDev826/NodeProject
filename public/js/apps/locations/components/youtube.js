function processDataYoutube(data){
    var dmn = 'YOUTUBE';
    ...
    if (location_info) {
        var pos = getShiftedLoc(latitude, longitude);
        var marker = new google.maps.Marker({
            position: pos,
            icon: '' + mapicon[dmn],
            map: map
        });
        window['markerhash'][latitude + ':' + longitude] = 'Y';
        window['bounds_map'].extend(pos);

        marker.meta                 = {};
        marker.meta.latitude        = latitude;
        marker.meta.longitude       = longitude;
        marker.meta.thumbnail_image = thumbnail_image;
        marker.meta.username        = username;
        marker.meta.fullname        = fullname;
        marker.meta.user_url        = user_url;
        marker.meta.title           = title;
        marker.meta.description     = description;
        marker.meta.link            = link;
        marker.meta.typ             = dmn;
        marker.meta.embed_video     = embed_video;
        marker.meta.posted_time     = posted_time;
        marker.meta.sentiment       = sentiment;
        marker.meta.content         = getContentYoutube(dmn, thumbnail_image, link, fullname, username, posted_time, sentiment, description, embed_video);

        google.maps.event.addListener(marker, 'mouseover', function() {
            showMarkerTooltip(marker);
        });
        addPostItemToList(getContentYoutube(dmn, thumbnail_image, link, fullname, username, posted_time, sentiment, description, embed_video));
    }
}
