...
initEventHandler = () => {
    ...
    $('#createForm').submit(function(event) {
        ...
        var param = {
            dictionaryID:   getURLParameter(location.href, 'dictionaryID'),
            name:           name,
            doc_link:       doc_link,
            access:         access,
        ...
        $.ajax({
            url: '/dashboard/settings/dictionaries/edit',
            data: param,
            type: 'POST',
            success: (res) => {
                if (res.success) {
...
loadValues = () => {
    $.ajax({
        ...
        type: 'POST',
        data: {dictionaryID: getURLParameter(location.href, 'dictionaryID')},
        success: (res) => {
            if (!res.success) return;

            var dictionary = res.dictionary;
            
            $('#name').val(dictionary.name);
            $('#doc_link').val(dictionary.doc_link);
            $('#access').val(dictionary.access);
            $('#license_type').val(dictionary.license_type);
            $('#license_method').val(dictionary.license_method);
            $('#license_cost').val(dictionary.license_cost);
            $('#description').val(dictionary.description);
            $('#tags').tokenfield('setTokens', dictionary.tags);
            $('#keywords').tokenfield('setTokens', dictionary.keywords);
